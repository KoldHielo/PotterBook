...
def set_headers(rend):
  rend['Content-Security-Policy'] = f"frame-ancestors 'self' {os.environ['TRUSTED_URIS']};"
  return rend
...
def profile(request):
  ...
  context = {
    ...
    'business_url': f'https://{user_profile.business_slug}.potterbook{settings.env}.kowd.uk/',
  ...
  if request.user.is_authenticated:
    if request.method == 'POST' and validate_post_data(request.POST):
      ...
      try:
        ...
        qr_img = generate_qr_code(f'https://{slug}.potterbook{settings.env}.kowd.uk/', f'{slug}-qr', 'png')
        ...
        updated['business_url'] = f'https://{user_profile.business_slug}.potterbook{settings.env}.kowd.uk/'
      except:
        pass
...
def home(request):
  ...
  rend = render(request, 'home/home.html', context)
  if request.user.is_authenticated == False:
    rend = set_headers(rend)
  return rend
      ...
      if appointment.exists() and intent_check.exists() is False and intent['amount'] == service.price and intent['currency'] == service.currency and intent['description'].split(';')[0] == service.service:
        if intent.status == 'requires_capture':
          ...
          qr_img = generate_qr_code(
            f'https://potterbook{settings.env}.kowd.uk' + reverse('verify_appointment', args=[verification_code]),
            verification_code,
            'png'
          )
          ...
          verification_url = f'https://potterbook{settings.env}.kowd.uk' + reverse('verify_appointment', args=[verification_code])
...
<html>
    ...
    <body style="background-color: #172145; border: 4px solid #263771;">
        ...
        <div style="text-align: center; width: 80%; margin: 30px auto; font-size: 1.4em; font-family: 'Karla', Arial; color: white;">
          ...
          send_mail(email_subject, email_message, 'potterbook@kowd.uk', [client_email], html_message=html_message)
          send_mail(
